...
describe("readServerConfig", () => {
  ...
  it("HTTPSの公開OriginではSecure Cookieを有効にする", () => {
    expect(
      readServerConfig({
        ...
        BFF_PUBLIC_ORIGIN: "https://library.example",
      }).cookieSecure,
    ).toBe(true);
  });
  ...
  it("loopback以外のHTTP公開Originを拒否する", () => {
    expect(() =>
      readServerConfig({
        SPRING_API_BASE_URL: "http://localhost:8080",
        BFF_PUBLIC_ORIGIN: "http://library.example",
      }),
    ).toThrow(InvalidServerConfigError);
  });
});
