...
- `MUST` 画面離脱または所有境界のアンマウントで破棄してよい状態に`useState`を使用し、複雑な局所遷移にだけ`useReducer`を使用する。
- `MUST NOT` 再読み込み後の復元が必要な状態を、揮発するClient Stateだけへ保存する。

## ブラウザ永続化
- `MAY` 再読み込み後も維持する必要があり、同一ブラウザだけでよく、消失または利用者による改ざんを許容できる非秘密のUI設定と下書きに`localStorage`を使用してよい。
- `MUST NOT` セッションID、Bearer、パスワード、個人情報、権限、認証判断、ブラウザへ公開しない内部情報を`localStorage`または`sessionStorage`へ保存する。
- `MUST NOT` `localStorage`の値を認証、認可、価格、在庫、予約成立などの業務判断の根拠にする。
- `MUST` `localStorage`のキー、読み取り、実行時検証、書き込み、削除を、状態を所有する機能の型付きブラウザストレージ境界へ集約する。
- `MUST NOT` コンポーネントから`localStorage`を直接操作し、保存キーまたはJSON形式を重複定義する。
- `MUST` 保存形式へバージョンを持たせ、読み取り時に型とバージョンを検証する。
- `MUST` JSON破損、未対応バージョン、期限切れ、容量超過、ブラウザ設定による利用不可を処理し、安全な初期値へ戻せるようにする。
- `MUST` 期限が必要な値へ失効日時と削除条件を定義し、不要になった値を削除する。
- `MUST` ブラウザ保存値をBFFまたはSpring Boot APIへ送る場合、未信頼入力としてサーバー側で再検証する。
- `MUST` ブラウザストレージが利用できない場合でも、認証、検索、予約の主要フローを成立させる。
...
- `MUST` ブラウザ永続化を追加する場合、正常な保存値、JSON破損、未対応バージョン、期限切れ、容量超過、利用不可から安全に回復することをテストする。
- `MUST` ブラウザストレージへ認証秘密、個人情報、権限が保存されないことを確認する。
