...
### BFF実装の濫用制限
- 未デプロイの単一BFFでは外部依存を追加せず、ログインとログアウトへ別々のプロセス内トークンバケットを適用する。
- 各バケットは20件の短時間バーストを許可し、毎秒10件を補充する（持続600件/分）。制限状態はプロセス再起動で失われる。
- ログインの制限超過は`rate_limited`と再試行秒数だけをAction結果へ返し、Spring Boot APIを呼び出さない。
- ログアウトは制限超過時もBFFセッション削除とCookie失効を先に完了し、API側Bearer失効を省略した場合は構造化ログへ安全なエラー分類だけを記録する。
- 共有ストア、複数インスタンス、信頼できるエッジ単位の制限はデプロイ方針決定時に再評価する。

### BFF公開Origin
- `BFF_PUBLIC_ORIGIN`をサーバー専用設定として検証し、HTTPSのときだけCookieへ`Secure`を付与する。
- HTTPはlocalhostまたはloopbackのOriginだけを許可し、その他のHTTP Originは起動時に拒否する。
