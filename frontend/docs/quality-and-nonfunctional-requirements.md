...
### Exportされた関数のテスト対象判断
- `MUST` 独立した振る舞い、入力検証、状態遷移、秘密情報境界、外部通信を持つ`export`関数は、対応するテストを持つ。
- `MUST` 純粋な計算、入力検証、状態遷移、エラー変換は単体テストで検証する。
- `MUST` API、Cookie、セッション、Server Actionなどの信頼境界は、API境界テスト、統合テスト、またはブラウザE2Eのいずれか適切な層で検証する。認証秘密を扱う境界は代表的な異常系も含める。
- `MUST` UIコンポーネントはStorybookまたはコンポーネントテストで表示状態と利用者操作を検証し、複数プロセスをまたぐ振る舞いはブラウザE2Eで検証する。
- `MAY` 単なる委譲、定数、型定義、生成型は、利用側のテスト、型検査、生成物ドリフト検査で契約を担保できる場合、専用テストを作成しなくてよい。
- `MUST NOT` TypeScriptの`export`有無だけをテスト要否の根拠にしない。独立した振る舞い、信頼境界、回帰リスクを基準に判定する。
- `MUST NOT` 同じ振る舞いを単体テストとE2Eで過度に重複させない。単体テストは分岐、境界テストは契約、E2Eは横断フローを主な責務とする。
