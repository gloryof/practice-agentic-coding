...
## 対応案
- `task/todo/2026-08-16-02-measure-agent-workflow-context-baseline.md`で定義した同一シナリオ、入力、区間、指標を使って改善後を再計測する。
- 各シナリオで、総コンテキスト、必要コンテキスト率、重複探索回数、実行レビュー数、処理時間、回帰検査結果、完了条件の見落とし有無を比較する。
...
## 確認方法
- 改善前後を同一条件で比較した結果が保存される。
- 総量だけでなく、必要コンテキスト率とMain Agent・レビュアーを合わせた全体コストを確認できる。
- 回帰ケースが成功し、必要な規則、レビュー、検証、完了条件の欠落がない。
