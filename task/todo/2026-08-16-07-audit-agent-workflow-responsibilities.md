...
2026-09-07時点で、`AGENTS.md`の入口をユーザーによるワークフロー選択へ変更し、選択一覧には整備済みのユーザーストーリー作成フローを登録した。既存のサーバー・フロントエンド実装フローは、削除済みの`agents/flows/implementation-task-flow.md`による領域分類、active TODO確認、仕様影響判定を前提としており、選択一覧への登録前に入口を整合する必要がある。当面、実装依頼は「ワークフローを適用しない」経路で共通規則・領域規則と必要な検証に従う。
...
- 実装フローを選択一覧へ登録する際は、両領域フローの共通入口への依存と`task/todo/README.md`などの旧入口参照を整理し、共通確認の担当と横断変更の実行順序を確定する。
